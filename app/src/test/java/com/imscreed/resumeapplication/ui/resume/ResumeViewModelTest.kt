...
import com.imscreed.resumeapplication.CoroutineTestRule
import com.imscreed.resumeapplication.OneTimeObserver
import com.imscreed.resumeapplication.base.GenericModel
import com.imscreed.resumeapplication.mock
...
class ResumeViewModelTest {
    ...
    private val observer: Observer<ResumeDataModel> = mock()
    ...
    @Test
    fun `verify Livedata returnsGivenValue`() {
        val resumeDataModelString =
        ...
        val expectedResume: ResumeDataModel =
            GenericModel.newInstance(resumeDataModelString, ResumeDataModel::class.java)

        val subject = viewModel.resumeLiveData

        coroutineRule.runBlockingTest {
            subject.postValue(expectedResume)
        }

        subject.observeOnce {
            println("Expected -> " + expectedResume.basics.name)
            println("Actual -> " + it.basics.name)
            assertEquals(expectedResume.basics.name, it.basics.name)
        }
    }
    ...
    private fun <T> LiveData<T>.observeForeverForTesting(
        block: () -> Unit) {
